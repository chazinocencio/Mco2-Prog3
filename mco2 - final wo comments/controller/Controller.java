...
AWAITING_P1_ABILITY_SELECTION, AWAITING_P2_ABILITY_SELECTION, SHOW_ROUND_RESULT, SHOW_BATTLE_RESULT
    ...
    // --- Add helper for magic item selection prompt ---
    ...
    // --- Add these variables for magic item selection state ---
    ...
    // --- Utility: Check if player has any usable magic items ---
        ...
        switch (e.getActionCommand()) {
            ...
            case "Confirm":
                ...
                if (inputState == InputState.SHOW_ROUND_RESULT) {
                    // After Hallff, return to main menu
                    intrfc.setGameAreaText("Which action do you want to do? (click Button below)");
                    inputState = InputState.NONE;
                    intrfc.showManageControls();
                    intrfc.clearUserAnswer();
                    ...
                    // 1) Handle the “which player” answer
                    if ("1".equals(ans)) {
                        ...
                        intrfc.clearUserAnswer();
                        // leave the controls visible so they can answer again
                    ...
                    // New branch for your second view‑mode question
                    switch (ans.toLowerCase()) {
                        case "1":
                   ...
                   String charName = ans; // whatever was typed in the text field
                    if (charName.isEmpty()) {
                ...
                model.setTempCharRace(selectedRace); // store race temporarily
                ...
                return;
                    ...
                    if(model.getTempCharRace() != Race.Gnome){
                        model.getTemPlayer().selectThreeAbilitiesGUI(selectedType); // or chosen class
                        ...
                        model.getTemPlayer().startGnomeAbilitySelection(selectedType); // or chosen class
                   ...
                   CharacterType originalType = model.getTempCharType();  
                    ...
                    if (idx == -1) {
                        ...
                        // Valid index → start editing abilities
                        editingIndex = idx;
                    ...
                    if (abilityIdx == -1) {
                        ...
                        // Now commit the new abilities into the selected character
                    ...
                    if (idx == -1) {
                        intrfc.appendGameAreaText("\nInvalid! Enter 1 - 4:");
                        intrfc.clearUserAnswer();
                        inputState = InputState.AWAITING_EDIT_ABILITY2_GNOME; // stay in the same state
                    } else {
                        intrfc.appendGameAreaText("\n" + model.getTemPlayer().selectGnomeAbilityByIndex(idx));
                ...
                if (index == -1) {
                    intrfc.appendGameAreaText("\nInvalid! Enter 1 - 4:");
                    intrfc.clearUserAnswer();
                    inputState = InputState.AWAITING_EDIT_ABILITY3_GNOME; // remain in same state
                } else {
                    ...
                    if (selectedType == null) {
                        ...
                        intrfc.clearUserAnswer();
                        inputState = InputState.AWAITING_EDIT_CHARTYPE_GNOME;  // stay in current input state
                    } else {
                        model.setTempCharTypeGnome(selectedType);
                        model.getTemPlayer().setGnomeFourthAbilityClass(selectedType);
                    ...
                    if (index == -1) {
                        ...
                        intrfc.appendGameAreaText("\nSelect ability #4:\n" + options);
                        intrfc.clearUserAnswer();
                        inputState = InputState.AWAITING_EDIT_ABILITY4_GNOME;  // remain in same state
                    } else {
                        intrfc.appendGameAreaText("\n" + model.getTemPlayer().selectGnomeAbilityByIndex(index));
                ...
                try {
                    ...
                    // Initialize fight state variables
            ...
            // --- FIGHT: P2 selects ability ---
                ...
                if (awaitingMagicItemP2) {
                    ...
                    if (selectedItem != null) {
                        ...
                        // Now resolve the round (no prompt needed here)
                        resolveFightRound();
                        return;
                    } else {
                        intrfc.appendGameAreaText("\nInvalid magic item selection. Try again:\n" + getMagicItemsPrompt(model.getp2()));
...
private void resolveFightRound() {
    // Always apply base +5 EP recharge and HP recharge at the start of the round
    ...
    // Passive magic items 
    // Amulet of Vitality: +20 max HP (once per round, can stack)
